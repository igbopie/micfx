/* test_comp.c — verificación offline del compresor (sin ALSA).
 *
 * Casos (seno 1 kHz, 48 kHz, attack 5 ms / release 100 ms):
 *   1. amplitud 0.5, thresh 0.25, ratio 4 -> pico ≈ 0.25+(0.5-0.25)/4 = 0.3125 (±10 %)
 *   2. amplitud 0.1 bajo el umbral -> pico ≈ 0.1 (±2 %)
 *   3. ratio 1.0 (bypass) con amplitud 0.5 -> pico ≈ 0.5 (±0.1 %)
 * Sale 0 si los tres cumplen, 1 si no.
 */
#include <math.h>
#include <stdio.h>
#include "dsp.h"
...
int main(void) {
    ...
    printf("comp 4:1//0.25: in=0.5 -> %.4f (esp 0.3125) | in=0.1 -> %.4f (esp 0.1)\n", p1, p2);
    printf("bypass 1:1: in=0.5 -> %.4f (esp 0.5)\n", p3);
